...
class AilangResNet(al.nn.Module):
    def __init__(self):
        ...
        downsample.weight = al.from_numpy(pd["downsample2.0.weight"])
        ...
        downsample.weight = al.from_numpy(pd["downsample3.0.weight"])
        ...
        downsample.weight = al.from_numpy(pd["downsample4.0.weight"])
...
def numeric_check(a, b: np.ndarray):
    return np.allclose(a.tolist(), b.tolist(), rtol=1e-04, atol=1e-06)
from ailang.util import check_intermediate

def test_resnet():
    a_model = AilangResNet().mlu()
